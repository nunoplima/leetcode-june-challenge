...
const findCheapestPrice = (n, flights, src, dst, K) => {
    // create a graph a populate it with the trips costs
    const graph = new Array(n);
    for (let r = 0; r < graph.length; r++) {
        graph[r] = new Array(n).fill(0);
    }
    flights.forEach(([from, to, cost]) => graph[from][to] = cost);

    // ~dijkstra, minheap, bsf
    let minHeap = [[src, graph[src], 0, 0]];
    while (minHeap.length) {
        const [city, neighbors, cCost, cK] = minHeap.shift();
        if (city === dst) {
            return cCost;
        } else {
            const formatedNeighbors = [];
            neighbors.forEach((cost, idx) => {
                if (cost && (cK <= K)) {
                    formatedNeighbors.push([idx, graph[idx], cCost + cost, cK + 1]);
                } 
            });
            minHeap = [...minHeap, ...formatedNeighbors].sort((a, b) => a[2] - b[2]);
        } 
    }
    return -1;
};
...
console.log(findCheapestPrice(4, [[0,1,1],[0,2,5],[1,2,1],[2,3,1]], 0, 3, 1)); // 6